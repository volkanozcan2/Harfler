...
export const generateLetterImage = async (context: SelectedContext): Promise<string> => {
  ...
  try {
    ...
    let prompt = "";
    if (isSoftG) {
      prompt = `A cute, colorful, cartoon-style illustration for a children's alphabet book. Show a ${context.englishTranslation} (${context.word}) with a large, friendly letter 'Ğ' integrated into the scene or resting on it. The style should be vibrant, simple, and rounded, suitable for toddlers. Pure white background, isolated on white.`;
    } else {
